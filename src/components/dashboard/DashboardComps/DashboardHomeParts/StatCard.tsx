...
const StatCard: React.FC<StatCardProps> = ({ title, value, link = "#" }) => {
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-zinc-100 flex flex-col gap-4 min-w-[200px] flex-1">
      ...
      <Link className="flex justify-between items-center text-zinc-400" href={""}dir='ltr'>
        <div className="flex items-center gap-1">
           <Image src={arrhome} alt="arrow"  className="opacity-100" />
        </div>
        
        <div  className="text-xs hover:text-zinc-800 transition-colors cursor-pointer">
          مشاهده
        </div>
      </Link>
    </div>
