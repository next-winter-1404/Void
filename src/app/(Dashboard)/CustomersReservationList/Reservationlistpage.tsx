...
function ActionMenu({ onDetail, onDelete }: { onDetail: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-7 z-50 bg-white border border-gray-100 rounded-xl shadow-lg min-w-[110px] py-1 overflow-hidden">
          <button
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm space-y-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <ActionMenu onDetail={onDetail} onDelete={onDelete} />
      </div>
                  ...
                  {bookings.length === 0 ? (
                    ...
                    <tr key={row.id ?? i} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
                      <td className="py-3 px-4">
                        <ActionMenu onDetail={() => setDetailBooking(row)} onDelete={() => setDeleteBooking(row)} />
                      </td>
